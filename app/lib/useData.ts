...
export interface UseDataResult {
  ...
  errors: Partial<Record<"daily" | "target" | "nu" | "training", string>>;
...
export function useDashboardData(): UseDataResult {
  ...
  const model = useMemo<DashboardModel | null>(() => {
    if (!daily.data && !target.data && !nu.data && !training.data) return null;
    try {
      return buildModel(
  ...
  }, [daily.data, target.data, nu.data, training.data]);
  ...
  const isLoading =
    (daily.isLoading || target.isLoading || nu.isLoading || training.isLoading) &&
    !model;
